...
// 1. UTILITY FUNCTIONS
// ════════════════════════════════════════════════════════
// typeName(), fmtDate(), fmtTime(), buildCommitMessage() moved to
// tests/suite.node.js (runNodeSuite) — they also run headless in Node.
...
await describe('saveEntry() merge – meals', async () => {
  await it('appends new meals to existing entry', async () => {
    ...
    // Pin the time so the merge sort is deterministic — addMeal() defaults the
    // time to "now", which sorts before the 08:00 breakfast when the suite runs
    // before 8 AM and flips the expected order.
    document.querySelector('#meals-container .ml-time').value = '12:00';
    const restore = mockSave();
    await saveEntry();
    restore();
...
// "renderPatterns() – sleep influence" and "nextDateStr() / dayPoorSleep()" moved
// to tests/suite.node.js (runNodeSuite) — they also run headless in Node.
